...
import java.util.concurrent.TimeUnit; //delay the println
...
public class Pokemon { // class
	...
	private static String[] strengths = { "electric, flying, and steel", "water and fairy",
			"bug, steel, grass, and ice", "ground, rock, and fire" };
	private static String[] weaknesses = { "ground", "flying, fire, psychic, and ice", "ground, rock, and water",
			"electric and grass" };
	private static String[] ability1 = { "lightning", "chlorophyll", "blaze", "torrent" };
	private static String[] ability2 = { "static", "overgrow", "solar", "raindish" };
	...
	public static void main(String[] args) throws InterruptedException {
		...
		hmap_health.put("Squirtle health: ", 44);
		...
		while (setRun(true)) { // loop condition
			...
			if (pokemon == 1) {
				...
				System.out.println("Strengths: " + strengths[0] + "\n"); // pokemon types that it can win against
				TimeUnit.SECONDS.sleep(1);
				System.out.println("Weaknesses: " + weaknesses[0] + "\n"); // pokemon types that it may or may not win
																			// against
				TimeUnit.SECONDS.sleep(1);
	...
	private void startGame() throws InterruptedException { // introduction to game and concept
		...
		TimeUnit.SECONDS.sleep(2);
		System.out.println(
				"You might be wondering, what are Pokémon? Well " + name + ", Pokémon is short for pocket monsters.\n");
		TimeUnit.SECONDS.sleep(3);
		System.out.println(
				"There are several categories of Pokémon which have advantages and disadvantages when battling other types.\n");
		TimeUnit.SECONDS.sleep(3);
		System.out.println(
				"Each creature possesses a different set of abilities. Now, are you ready to choose your Pokémon and fight? (* 0 *) \n");
		TimeUnit.SECONDS.sleep(2);
		...
		if (start == 0) {
			...
			System.out.println("Choose your pokemon! (> v <) \n"); // I included keyboard faces for decoration
			TimeUnit.SECONDS.sleep(1);
	...
	private void continueGame() {
		...
		try {
			System.in.read();
		} // reads user input (key)
		catch (Exception a) {
		}
	}

	private void battlefield(String me, String enemy, String ability1, String ability2, String enemyAbility1,
			String enemyAbility2, int attack, int enemyAttack, int defense, int enemyDefense, int health,
			int enemyHealth) throws InterruptedException {
		...
		enemyNewHealth = enemyHealth - attack + enemyDefense; // determines enemy's new health by subtracting attack
															  // adding it's defense
		...
		System.out.println("---------------------------------------------------------------------------------------\n");
		System.out
				.println("But " + enemy + "'s " + enemyAbility1 + " cuts " + me + "'s " + specialAbility + "! :(( \n");
		TimeUnit.SECONDS.sleep(2);
		...
		TimeUnit.SECONDS.sleep(2);
		newHealth = health - enemyAttack + defense; // determines new health by subtracting enemy's attack and adding
													// it's defense
		...
		TimeUnit.SECONDS.sleep(3);
		System.out.println(
				"*GRRRRRR* Attack now to finish " + enemy + " off. Choose an ability: " + ability1 + " or " + ability2);
		TimeUnit.SECONDS.sleep(2);
	...
	private void determineWinner(int attack, int enemyAttack, int defense, int enemyDefense, int health,
			int enemyHealth) {
		newHealth = health - enemyAttack + defense;
		enemyNewHealth = enemyHealth - attack + enemyDefense;
		System.out.println("---------------------------------------------------------------------------------------\n");
		if (newHealth > 0 && newHealth > enemyNewHealth) {
			System.out.println("You won! (* o *) \n");
		} else {
			System.out.println("You lost! 8(> _ <)8 \n");
		}
	}
	...
	public static boolean setRun(boolean run) {
		Pokemon.run = run;
		return run;
	}
